...
\begin{list}{X:}{\setlength{\labelsep}{5mm}}
    ...
    \item[\textbf{R}:] Yes, sure. If you want to do something based on the bgRFC, you have to do certain things beforehand. For example you have to configure inbound destinations and also tune do some configuration for the framework. This is more or less from the on premise system, where the customer has a maintenance team. So for each product, they also have online documentation to tell the customer support team what what steps should be taken to configure bgRFC correctly, and then you can use the bfRFC as technologies to develop your own code. While this part is quite simple, because it is just a syntax where you you call something in the background unit but the function module itself, you have to develop beforehand. So in my opinion is it's quite simple, it's just a couple lines of code.
    ...
    \item[\textbf{R}:] So basically the bgRFC is shipped in default configuration with any ABAP System. 
\end{list}